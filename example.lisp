...
(defvar *label1* (make-label "hello world!" (vec2 100 400)))
(defvar *label2* (make-label "hello larger world!" (vec2 100 300) :size 24))
(defparameter *panel* (make-panel (vec2 100 100) :width 200 :height 200 :rounding 10
                            :children (list (make-label "This is a panel!" (vec2 25 150)
                                                        :size 20 :color gamekit.colors:+white+)
                                            (make-button (vec2 25 100)
                                                         :label (make-label "Panel Button!" (vec2 0 0)
                                                                            :size 20 :color gamekit.colors:+white+)
                                                         :fill-color gamekit.colors:+deeppink+
                                                         :hover-color gamekit.colors:+hotpink+
                                                         :pressed-color gamekit.colors:+red+))))
(defparameter *btn1* (make-button (vec2 350 150)
                                  :on-click #'click-me-callback
                            :label (make-label "click me!" (vec2 0 0) :size 48
